...
export class Releases {
   ...
   public async latest(request: Request, env: Environment): Promise<SemVer> {
   ...
   public async all(request: Request, env: Environment): Promise<Array<SemVer>> {
      return (await this._all(request, env)).map(v => this._toSemver(v));
   }

   private async _all(request: Request, env: Environment): Promise<Array<string>> {
      for (let i = 0; i < maxRetrieveTries; i++) {
   ...
   public async has(request: Request, env: Environment, version: string | SemVer): Promise<boolean> {
      const all = await this._all(request, env);
      return all && all.indexOf(typeof version === 'string' ? version : version.toString()) >= 0;
   }

   private async _retrieveReleases(_request: Request, env: Environment) {
      ...
      const allSorted = all.map(v => v.toString())
         .sort()
         .reverse();
